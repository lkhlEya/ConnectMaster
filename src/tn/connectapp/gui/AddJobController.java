...
public class AddJobController implements Initializable {
    ...
    @FXML
    private final ComboBox cbAgreementType = new ComboBox();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // TODO  
        ObservableList<String> matrialStatusList = FXCollections.observableArrayList("CIVP", "CDI", "CDD");
        cbAgreementType.setItems(matrialStatusList);
    ...
    private void save(ActionEvent event) {
        ...
        String type = (String) cbAgreementType.getValue();
        String company = tfCompany.getText();
        String ref = tfRef.getText();
        String pren = tfField.getText();
        String title = tfTitle.getText();
        String start_date = String.valueOf(dpStartDate.getValue());
        String taDescriptionription = taDescription.getText();
        String position = tfPosition.getText();

        if (type == null || company.isEmpty() || ref.isEmpty() || pren.isEmpty() || title.isEmpty() || start_date.isEmpty() || position.isEmpty() || taDescriptionription.isEmpty()) {
    ...
    private void getQuery() {
        ...
        if (update == false) {
            ...
            query = "INSERT INTO work ( `ref`, `company`, `field`, `title`, `description`, `start_date`, `agreementType`, `position`, `status` ) VALUES (?,?,?,?,?,?,?,?,?)";
...
//                    + "`status`=?WHERE ref = '" + ref + "'";
    ...
    private void insert() {
        ...
        try {
            ...
            preparedStatement.setString(2, tfCompany.getText());
            preparedStatement.setString(3, tfField.getText());
            preparedStatement.setString(4, tfTitle.getText());
            preparedStatement.setString(5, taDescription.getText());
            preparedStatement.setString(6, String.valueOf(dpStartDate.getValue()));
            preparedStatement.setString(7, (String) cbAgreementType.getValue());
            preparedStatement.setString(8, tfPosition.getText());
